...
- Updated example scripts: CUDA quick test, systemd GPU service Makefile/README now use Docker syntax; added `examples/postgres-docker-compose/` sample stack.
...
### KIWI ISO Layer
- `profile/config.kiwi.xml`: annotated that Docker is layered post-install (no runtime baked into ISO); Podman packages removed from image layer.
...
- `docs/directory-tree.md`: refreshed tree to reflect new/removed files.
...
- Add validation scripts to `tools/`
...
**Lefthook:**
- [ ] Create `lefthook.yml` at repo root
- [ ] Create `tools/check-anti-patterns.sh`
- [ ] Create `tools/check-layer-assignments.sh`
