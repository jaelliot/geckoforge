...
## Part 2: OPA Policy Integration

### Overview
Added Open Policy Agent (OPA) style policy checks to enforce compliance with the KIWI NG audit findings. The policies prevent regressions by blocking commits that violate critical rules.

### Files Created
| File | Description |
|------|-------------|
| `policies/opa/geckoforge.rego` | Rego policy file (for reference, not currently evaluated) |
| `tools/opa-check.sh` | Policy enforcement script with bash-based checks |
| `docs/opa-integration.md` | Documentation for OPA integration |

### Files Modified
| File | Changes |
|------|---------|
| `lefthook.yml` | Added `opa-policy` pre-commit hook and `opa-full-check` pre-push hook |

### Policy Coverage (from Audit Report)

**Critical (Blocks Commits):**
- ISSUE-001: Config file naming (`config.xml` not `config.kiwi.xml`)
- ISSUE-002: Missing `<contact>` in `<description>`
- ISSUE-003: Deprecated `<files>` element
- ISSUE-004: Deprecated `hybrid` attribute
- ISSUE-005: Wrong `<package>` syntax

**High Severity (Blocks Commits):**
- Podman usage (Docker only)
- Podman GPU syntax (`--gpus all` not `--device nvidia.com/gpu`)
- TeX scheme-full (use scheme-medium)
- Wrong package managers (use zypper)

**Medium Severity (Warnings):**
- ISSUE-012: Services in `multi-user.target.wants/` should be symlinks

**Low Severity (Advisory):**
- ISSUE-019: Missing firmware packages for laptop support
